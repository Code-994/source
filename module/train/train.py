...
def train():
    ...
    if num_classes > 0:
        model.init_cls_head(num_classes=num_classes)
        # 修复：把 class_list 挂到 base model 上。
        # lora_recipe.save() 会引用 base.class_list 写 class_list.json（评估时 cls_head
        # idx→类名 映射依赖它），但此前从未赋值，导致 class_list.json 缺失。
        # 注意需挂到 base：PeftModel 属性赋值不会转发到底层模型。
        from peft import PeftModel
        _base = model.get_base_model() if isinstance(model, PeftModel) else model
        _base.class_list = data_module["train_dataset"].class_list
    else:
    ...
    trainer = LLaVATrainer(
        model=model,
        # tokenizer=tokenizer,  # transformers 5.x 中改名为 processing_class
