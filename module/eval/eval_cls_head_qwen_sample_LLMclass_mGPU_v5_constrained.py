...
# ============================================================
# v5：约束解码（lm-format-enforcer）—— 生成时强制输出合法 JSON（符合报告 schema）
# 白名单机制：每步只放行"保持 JSON 合法"的 token → 从源头杜绝一切格式错误（引号/多值/跑偏等）。
# class 字段约束为 class_list 枚举 → 保证 class 永远是有效类别。
# ============================================================

def _report_json_schema(class_list):
    lmh = {"type": "string", "enum": ["low", "mid", "high"]}
    return {
        "type": "object",
        "properties": {
            "class": ({"type": "string", "enum": list(class_list)} if class_list else {"type": "string"}),
            "traits": {
                "type": "object",
                "properties": {
                    "has_tls_record":        {"type": "boolean"},
                    "has_http_method":       {"type": "boolean"},
                    "ascii_ratio_bucket":    lmh,
                    "entropy_bucket":        lmh,
                    "zero_pad_ratio_bucket": lmh,
                },
            },
        ...
        "required": ["class", "traits", "evidence", "description", "notes"],
    }


def build_json_enforcer(tokenizer, class_list):
    """返回 prefix_allowed_tokens_fn（传给 generate()），强制输出符合报告 schema 的合法 JSON。
    lm-format-enforcer 不可用或构建失败时返回 None（v5 退化为无约束，等同 v4）。"""
    try:
        import transformers
        from transformers import PreTrainedTokenizerBase
        import transformers.tokenization_utils as _tu
        _tu.PreTrainedTokenizerBase = PreTrainedTokenizerBase   # 兼容 transformers 5.x 的导入位置
        from lmformatenforcer import JsonSchemaParser
        from lmformatenforcer.integrations.transformers import (
            build_transformers_prefix_allowed_tokens_fn, build_token_enforcer_tokenizer_data,
        )
        tk_data = build_token_enforcer_tokenizer_data(tokenizer)
        parser  = JsonSchemaParser(_report_json_schema(class_list))
        return build_transformers_prefix_allowed_tokens_fn(tk_data, parser)
    except Exception as e:
        print(f"[WARN] 约束解码构建失败，退化为无约束(等同 v4): {type(e).__name__}: {e}")
        return None
...
def evaluate(
    ...
    # ── v5：构建 JSON schema 约束解码器（一次性，~3s）──
    prefix_fn = None
    if constrained:
        prefix_fn = build_json_enforcer(tokenizer, class_list)
        print(f"[{device}] 约束解码: {'✅ 已启用（强制合法 JSON）' if prefix_fn else '❌ 未启用（退化为无约束）'}")
    ...
    for batch in tqdm(dataloader, desc=f"[{device}] Evaluating"):
        ...
        if prefix_fn is not None:
            gen_kwargs["prefix_allowed_tokens_fn"] = prefix_fn   # v5：约束解码
...
def main():
    ...
    parser.add_argument("--constrained", action="store_true",
                        help="v5：约束解码，生成时强制输出符合报告 schema 的合法 JSON（class 约束为有效类别枚举），从源头杜绝格式错误")
