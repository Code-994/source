...
v5 改动（此版本）：
5. 修复 CLS Head 类名映射：不再使用硬编码的 NETMAMBA_IDX_TO_CLASS（仅12个USTC类别），
   改为从 eval JSONL 数据动态构建 class_list（sorted，与训练时 class_to_idx 一致）
...
def load_model(checkpoint_path: str, device: str = "cuda", vision_tower_path: str = None):
    ...
    # 当 user_text 已由数据集填充时不需要此 token，跳过断言
    if not isinstance(pid, int):
        print(f"[{device}] WARNING: <cls_placeholder> not in tokenizer (pid={pid}), cls injection via default template disabled.")
        pid = None
...
def evaluate(
    ...
    for batch in tqdm(dataloader, desc=f"[{device}] Evaluating"):
        ...
        pred_classes = [
            class_list[i.item()] if class_list and 0 <= i.item() < len(class_list)
...
def worker_process(rank: int, args, sampled_data: List[Dict], tmp_dir: str, class_list: list = None):
...
def main():
    ...
    # ── 构建 class_list（idx → class name，与训练时 class_to_idx 一致）──
    # 优先从 checkpoint 目录加载 class_list.json，否则从数据动态构建（sorted）
    class_list = None
    cl_json = os.path.join(args.checkpoint_path, "class_list.json")
    ...
    if class_list is None:
        ...
        print(f"[INFO] class_list built from data: {len(class_list)} classes (sorted)")
    ...
    # ── 多卡模式：spawn 子进程，每张卡处理一个切片 ──
        ...
        for rank in range(num_gpus):
            p = mp.Process(
                ...
                args=(rank, args, sampled_data, tmp_dir, class_list),
            )
            p.start()
            processes.append(p)
