...
Usage:
  ...
  # With ROUGE-L and BERTScore (requires: pip install rouge-score bert-score):
  python evaluate_final.py --input predictions.jsonl --output results.json --text

  # With a local roberta-large checkpoint for BERTScore reproducibility:
  python evaluate_final.py --input predictions.jsonl --output results.json \
      --text --bert-model-path /path/to/roberta-large
...
Output JSON structure:
  {
    "structured": {
      "n_total": ...,
      "n_structured": ...,
      "structured_output_rate": ...,      # JSON Valid%
    ...
    "text_metrics": {                     # null if --text not specified
      "n_structured": ...,
      "cls_accuracy": ...,                # JClsAcc%: class label match rate
      "evidence_rouge_l": ...,            # macro-avg ROUGE-L F1 on evidence field
      "description_rouge_l": ...,         # macro-avg ROUGE-L F1 on description field
      "evidence_bertscore_f1": ...,       # macro-avg BERTScore F1 on evidence (roberta-large, layer 17)
      "description_bertscore_f1": ...     # macro-avg BERTScore F1 on description
    },
...
def _join_evidence(d: dict) -> str:
    """Concatenate evidence list into a single string for metric computation."""
    ev = d.get("evidence", [])
    if isinstance(ev, list):
        return " ".join(str(e) for e in ev)
    return str(ev) if ev else ""


def _get_description(d: dict) -> str:
...
def compute_structured(records: list[dict]) -> dict:
    ...
    for r in struct:
        _t = r["gt"].get("traits")
        all_keys.update((_t if isinstance(_t, dict) else {}).keys())
    ...
    for tk in sorted(all_keys):
        pred_vals, gt_vals = [], []
        for r in struct:
            _pt = r["pred"].get("traits")
            _gt = r["gt"].get("traits")
            pv = (_pt if isinstance(_pt, dict) else {}).get(tk)
            gv = (_gt if isinstance(_gt, dict) else {}).get(tk)
    ...
    consistency_scores = []
    for r in struct:
        _tp      = r["pred"].get("traits")
        traits_p = _tp if isinstance(_tp, dict) else {}
...
# ── Part 2: Text Generation Metrics (ROUGE-L + BERTScore) ─────────────────────

def compute_text_metrics(records: list[dict],
                         bert_model_path: Optional[str] = None) -> dict:
    """
    Compute ROUGE-L and BERTScore for evidence and description fields,
    plus JSON Classification Accuracy (JClsAcc%).

    Paper details (Section V-A3):
      - ROUGE-L: F1 of longest common subsequence (rouge_score library).
      - BERTScore: roberta-large, num_layers=17, macro-averaged F1.
        Pass --bert-model-path to load from a local checkpoint for reproducibility.
      - JClsAcc%: proportion of structured outputs where pred["class"] matches gt["class"].
    """
    try:
    ...
    n = len(struct)
    if n == 0:
        print("[WARN] No structured predictions found for text metrics.", file=sys.stderr)
        return {"n_structured": 0}

    # ── Classification Accuracy (JClsAcc%) ────────────────────────────────────
    cls_correct = sum(
        1 for r in struct
        if (r["pred"].get("class") or "").strip().lower() ==
           (r["gt"].get("class")   or "").strip().lower()
    )
    cls_accuracy = cls_correct / n

    # ── Collect paired texts ───────────────────────────────────────────────────
    pred_ev   = [_join_evidence(r["pred"]) for r in struct]
    gt_ev     = [_join_evidence(r["gt"])   for r in struct]
    pred_desc = [_get_description(r["pred"]) for r in struct]
    gt_desc   = [_get_description(r["gt"])   for r in struct]

    # ── ROUGE-L ───────────────────────────────────────────────────────────────
    _scorer = _rouge_lib.RougeScorer(["rougeL"], use_stemmer=False)

    ev_rouge_l = float(np.mean([
        _scorer.score(ref, hyp)["rougeL"].fmeasure
        for hyp, ref in zip(pred_ev, gt_ev)
    ]))
    desc_rouge_l = float(np.mean([
        _scorer.score(ref, hyp)["rougeL"].fmeasure
        for hyp, ref in zip(pred_desc, gt_desc)
    ]))

    result: dict = {
        "n_structured":        n,
        "cls_accuracy":        cls_accuracy,
        "evidence_rouge_l":    ev_rouge_l,
        "description_rouge_l": desc_rouge_l,
    }
    ...
    try:
        ...
        # roberta-large, num_layers=17 (paper Section V-A3)
        _model = bert_model_path if bert_model_path else "roberta-large"
        _bs_kwargs = dict(model_type=_model, num_layers=17,
                          verbose=False, batch_size=64)

        print(f"Computing BERTScore (model={_model}, num_layers=17) "
              f"on {n} samples ...", flush=True)

        # bert_score 遇到空字符串会调用 tokenizer.build_inputs_with_special_tokens([])
        # 该方法在 transformers 5.x Fast tokenizer 上已移除，用占位符替换空字符串
        # _, _, F1_ev = _bert_score_fn(pred_ev, gt_ev, **_bs_kwargs)
        _pred_ev_safe  = [s if s.strip() else "[empty]" for s in pred_ev]
        _gt_ev_safe    = [s if s.strip() else "[empty]" for s in gt_ev]
        _pred_desc_safe = [s if s.strip() else "[empty]" for s in pred_desc]
        _gt_desc_safe   = [s if s.strip() else "[empty]" for s in gt_desc]

        _, _, F1_ev = _bert_score_fn(_pred_ev_safe, _gt_ev_safe, **_bs_kwargs)
        result["evidence_bertscore_f1"] = float(F1_ev.mean())

        _, _, F1_desc = _bert_score_fn(_pred_desc_safe, _gt_desc_safe, **_bs_kwargs)
        result["description_bertscore_f1"] = float(F1_desc.mean())

    except ImportError:
        print("[WARN] bert-score not installed. Skipping BERTScore. "
              "Run: pip install bert-score", file=sys.stderr, flush=True)
    except Exception as exc:
        print(f"[WARN] BERTScore failed: {exc}", file=sys.stderr, flush=True)

    # ── Summary print ─────────────────────────────────────────────────────────
    print(f"\n── Text Metrics (n={n}) ──────────────────────────────")
    print(f"  JClsAcc%          : {cls_accuracy:.4f}")
    print(f"  Evidence  ROUGE-L : {ev_rouge_l:.4f}")
    print(f"  Desc      ROUGE-L : {desc_rouge_l:.4f}")
    if "evidence_bertscore_f1" in result:
        print(f"  Evidence  BERTScore F1: {result['evidence_bertscore_f1']:.4f}")
        print(f"  Desc      BERTScore F1: {result['description_bertscore_f1']:.4f}")
    print("─" * 52, flush=True)
...
def main():
    ...
    parser.add_argument("--text",            action="store_true",
                        help="Compute ROUGE-L and BERTScore (requires rouge-score, bert-score)")
    parser.add_argument("--bert-model-path", default=None,
                        help="Local path to roberta-large checkpoint for reproducible BERTScore. "
                             "Defaults to downloading roberta-large from HuggingFace.")
    # LLM-as-Judge
    ...
    text       = compute_text_metrics(records, bert_model_path=args.bert_model_path) \
    ...
    results = {"structured": structured, "text_metrics": text, "judge": judge}
