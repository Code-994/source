...
# USTC-TFC-2016 12类 MambaNetBurst checkpoint 的 idx→类名映射（按 dataset_sampled1 字母顺序）
NETMAMBA_IDX_TO_CLASS = {
    0: "BitTorrent",
    1: "Facetime",
    2: "Geodo",
    3: "Gmail",
...
class TinyLlavaPreTrainedModel(PreTrainedModel):
    ...
    def _supports_sdpa(self):
        # language_model 在 super().__init__() 之后才初始化，__init__ 期间返回 False 避免 AttributeError
...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    ...
    def init_cls_head(self, num_classes: int, lambda_cls: float = 0.1):
        ...
        print(f"  [CLS_HEAD] Initialized: hidden_size={hidden_size}, num_classes={num_classes}, lambda_cls={lambda_cls}")
    ...
    def tie_weights(self, **kwargs):
        # transformers 5.x 新增 recompute_mapping 参数，透传 kwargs 兼容新旧版本
    ...
    def forward(
        ...
        # tune_type_llm=frozen 时跳过 gen_loss：让 connector 只接收 cls_loss 梯度。
        # frozen 模式下 gen_loss 梯度仍会流入 connector，会淹没 cls_loss 信号；
        # 置零后 Phase 1 预热变为纯分类驱动，不影响 lora/full 等其他训练模式。
        llm_frozen = (getattr(self.config, 'tune_type_llm', 'lora') == 'frozen')
        if llm_frozen and output.loss is not None:
            output.loss = output.loss * 0.0

        # 🚀 措施 C：对 JSON 输出前 20 个 Token（即类别字段）施加动态加权 Loss
        if self.training and not llm_frozen and labels is not None and output.logits is not None:
