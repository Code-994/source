...
# __init__.py 自动 import 该目录所有文件，包括本文件。
# models_mamba（NetMamba/Mamba-1 实现）→ mamba_ssm.mamba3 → triton.set_allocator
# 在 mamba_ssm 2.3.2 + triton 3.0 下会崩溃，用 try/except 隔离，
# 不影响 mambanetburst 等其他 VisionTower 的正常注册。
try:
    ...
    import sys as _sys
    if '/root/autodl-tmp/NetMamba/src' not in _sys.path:
        _sys.path.insert(0, '/root/autodl-tmp/NetMamba/src')
    from models_mamba import create_block, RMSNorm, rms_norm_fn, StrideEmbed
    try:
        from util.pos_embed import interpolate_pos_embed
    except ImportError:
        from timm.layers import resample_abs_pos_embed as interpolate_pos_embed
    _NETMAMBA_AVAILABLE = True
except Exception as _netmamba_err:
    import warnings
    warnings.warn(f"[netmamba] 导入失败，NetmambaVisionTower 不可用: {_netmamba_err}")
    _NETMAMBA_AVAILABLE = False
