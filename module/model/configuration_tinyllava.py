...
class TinyLlavaConfig(PretrainedConfig):
    ...
    def _load_vision_config(self, vision_config=None):
        ...
        if vt in [None, '']:
            ...
            return
        elif vt == 'mambanetburst':
            # MambaNetBurst: d_model=256, outputs (B, 400, 256) after pooling
            self.vision_config = DummyVisionConfig()
            self.vision_config.model_name_or_path = 'mambanetburst'
            self.vision_config.model_name_or_path2 = ''
            self.vision_hidden_size = 256  # MambaNetBurst d_model
            return
        else:
