...
class BaseTrainingRecipe:
    ...
    def add_args(self, model_args):
        ...
        # 直接指定视觉塔路径时覆盖上方派生路径（支持从独立分类器 checkpoint 初始化）
        direct_vt_path = getattr(self.training_arguments, 'pretrained_vision_tower_path', None)
        if direct_vt_path is not None:
            model_args['vision_tower']['pretrained_vision_tower_path'] = direct_vt_path
        # 直接指定 connector 路径时覆盖派生路径（Phase 2 单独加载 connector 而不加载 LLM）
        direct_connector_path = getattr(self.training_arguments, 'pretrained_connector_path', None)
        if direct_connector_path is not None:
            model_args['connector']['pretrained_connector_path'] = direct_connector_path
        return model_args
