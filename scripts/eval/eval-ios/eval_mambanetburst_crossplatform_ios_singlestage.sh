#!/bin/bash
# ==============================================================================
# iOS 单阶段联合训练评估（路线 1，v3-native encoder）
# 对应训练：scripts/train/train_mambanetburst_crossplatform_ios_singlestage.sh
# 输出目录：output/Crossplatform-iOS/mambanetbust_lora_full
#
# 注：单阶段 = base Qwen + 全新 LoRA（适配全在 adapter，未合并进 language_model），
#     所以 eval 加载 base+adapter 正好对，无两阶段 full 的合并权重加载坑。
#
# 跑两步：① 生成+分类指标  ② 文本指标(ROUGE-L/BERTScore)
# ==============================================================================

export PYTHONPATH="/root/autodl-tmp/mmTraffic:/root/autodl-tmp/MambaNetBurst/src:${PYTHONPATH}"
export TORCH_DTYPE=bfloat16

RUN_DIR="/root/autodl-tmp/mmTraffic/output/Crossplatform-iOS/mambanetbust_lora_full"
...
echo ">>> [iOS 单阶段 Eval / 路线1] (with cls_head fallback)"
echo "    Checkpoint  : $CHECKPOINT_PATH"
echo "    VT ckpt     : $VT_CKPT (v3-native)"
