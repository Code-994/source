#!/bin/bash
# ==============================================================================
# CrossPlatform-Android 单阶段（mambanetburst_lora）评估 —— v4 前缀强制重生成兜底
# 对比对象：双阶段 phase2_direct 的 v4（eval-phase2-v4-regen.sh）
#
# --regen_on_fallback：LLM 未输出 class 时，用 cls_head 兜底类别作 JSON 前缀强制
# 重生成整条报告 → evidence/description 随之自洽。纯 LLM 指标读 llm_class_raw（不污染）。
...
RUN_DIR="/root/autodl-tmp/mmTraffic/output/Crossplatform-Android/mambanetburst_lora"
...
echo ">>> [Android 单阶段 Eval v4] cls_head 兜底 + 前缀强制重生成报告"
