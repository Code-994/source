#!/bin/bash
# 推理评估: MambaNetBurst + mmTraffic on CrossPlatform-Android
# 使用 eval_cls_head_qwen_sample_LLMclass_mGPU_v2.py（JSON 格式约束）
# 对应训练: train_mambanetburst_crossplatform_android.sh
# max_new_tokens=400: target 最长 278 tokens，400 有充足余量
...
OUTPUT_DIR="/root/autodl-tmp/mmTraffic/output/Crossplatform-Android/mambanetburst_lora/eval_results"

mkdir -p "$OUTPUT_DIR"

echo ">>> [Eval] MambaNetBurst + mmTraffic on CrossPlatform-Android"
