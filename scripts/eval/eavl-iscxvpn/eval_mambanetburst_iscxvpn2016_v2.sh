#!/bin/bash
# 实验1推理评估 v2: MambaNetBurst + mmTraffic on ISCXVPN2016 BGTD
# 与 v1 的区别：使用 eval_cls_head_qwen_sample_LLMclass_mGPU_v2.py
#              在 user_text 末尾追加 JSON 格式约束
# 输出目录：eval_results1/
...
VT_CKPT="/root/autodl-tmp/mmTraffic/output/ISCXVPN2016/mambanetburst_lora/vision_tower/pytorch_model.bin"
...
OUTPUT_DIR="/root/autodl-tmp/mmTraffic/output/ISCXVPN2016/mambanetburst_lora/eval_results1"

mkdir -p "$OUTPUT_DIR"

echo ">>> [Eval1-v2] MambaNetBurst + mmTraffic on ISCXVPN2016 (JSON format constrained)"
