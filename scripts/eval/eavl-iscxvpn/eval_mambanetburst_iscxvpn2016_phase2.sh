#!/bin/bash
# ==============================================================================
# ISCXVPN2016 双阶段（phase1 → phase2_direct）训练后的评估
...
# 正好对，无合并权重坑（见 docs §9.3/§9.6），直接评估（生成+分类 → 文本指标）。
#
# 评估内容：
#   - JClsAcc：从 LLM 生成 JSON 解析的类别准确率（真指标）
#   - +cls_head 兜底：LLM 漏 class 字段时用 cls_head argmax 顶替
#   - JSON 合法率 / 字段完整率
#   - Evidence / Description 的 ROUGE-L + BERTScore
# ==============================================================================
...
echo ">>> [ISCXVPN phase2 Eval] (with cls_head fallback)"
