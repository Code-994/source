#!/bin/bash
# 实验3推理评估 v1: MambaNetBurst + mmTraffic on USTC-TFC-2016
# 使用原始推理脚本 eval_cls_head_qwen_sample_LLMclass_mGPU.py（无 JSON 格式约束）
# 输出目录：eval_results/
...
OUTPUT_DIR="/root/autodl-tmp/mmTraffic/output/USTC-TFC-2016/mambanetburst_lora/eval_results"

mkdir -p "$OUTPUT_DIR"

echo ">>> [Eval3-v1] MambaNetBurst + mmTraffic on USTC-TFC-2016 (no format constraint)"
