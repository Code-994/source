#!/bin/bash
# ==============================================================================
# ISCXVPN2016 — phase1 → 直接 phase2_direct（不冻、normal lr，单 run 联合训练）
# ------------------------------------------------------------------------------
...
# ISCXVPN 只有 7 类，预期稳定收敛。
#
# 用法：bash scripts/train/iscxvpn/train_mambanetburst_iscxvpn2016-phase2_direct.sh
# 前置：先跑完 train_mambanetburst_iscxvpn2016-phase1.sh（产出 phase1/{vision_tower,connector,cls_head}）
# ==============================================================================
...
PHASE1_DIR="/root/autodl-tmp/mmTraffic/output/ISCXVPN/phase1"
OUTPUT_DIR="/root/autodl-tmp/mmTraffic/output/ISCXVPN/phase2"
...
echo ">>> [ISCXVPN phase2] 加载 phase1 预热权重 + 直接全解冻 + normal lr 5e-5"
